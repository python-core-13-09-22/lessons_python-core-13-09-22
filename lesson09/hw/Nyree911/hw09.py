...
class CheatSheet():
    def __init__ (self, subject, number):
        ...
        self.number = number
    def __str__ (self):
        return f"{self.subject} subject cheetsheet, number {self.number}"
class PaperCheatSheet(CheatSheet):
    def __init__(self, size, hide):
        self.size = size
        self.hide = hide
    def __str__ (self):
        return f" {self.size} size, hiidden {self.hide}"
class ElectronicCheatSheet(CheatSheet):
    def __init__ (self, hub):
        self.hub = hub
    def __str__(self):
        return f" on {self.hub}"
# Визначити тип «`Студент`»(ПІП, група, список шпаргалок)
class Student(CheatSheet):
    def __init__(self, name, group, cheatSheet, type_of_cheat):
        self.name = name
        self.group = group
        self.lst_CheatSheet = cheatSheet
        self.type_of_cheat = type_of_cheat
    def __str__(self):
        return f"{self.name}, from {self.group} group, with {self.lst_CheatSheet},{self.type_of_cheat}"
    def ShowCheat(self):
        return f'{self.lst_CheatSheet},{self.type_of_cheat}'
...
for _ in range (5):
    lst_students.append(Student(students[randrange(0,11)], randrange(1,20), CheatSheet(subjects[randrange(0,6)], randrange(0,50)), PaperCheatSheet (size[randrange(0,2)], places_of_hide[randrange(0,4)])))
for _ in range (5):
    lst_students.append(Student(students[randrange(0,11)], randrange(1,20), CheatSheet(subjects[randrange(0,6)], randrange(0,50)), ElectronicCheatSheet (hubs[randrange(0,4)])))
...
all_cheatSheets.sort()
for i in all_cheatSheets:
    print (i)
